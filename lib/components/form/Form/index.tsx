...
import { cls, useEventListener, useFunction, useResizeObserver } from '../../../utils'
  ...
  useEventListener('ui-input-change', async (e) => {
    ...
    if (key) {
      ...
      if (Object.hasOwn(error, key) && validateRef.current[key]) {
        const message = await validateRef.current[key](value)
      ...
      onSubmit={async (e) => {
        ...
        await Promise.all(
          Object.entries(validateRef.current).map(async ([name, validate]) => {
            const message = await validate(values?.[name])
            if (message) {
              error[name] = message
            }
          })
